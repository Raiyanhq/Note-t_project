from flask import Flask, request_started, jsonify
import requests
import numpy as np
...
# Function to get answer from specified API database
def get_answer_from_api(question):
    database_api_url = 'https://api.spacexdata.com/v5/launches/latest'
...
def answer():
    question = requests.args.get('question')
    if not question:
        return jsonify({'error': 'No question provided'}), 400

    # Get answer from specified API database
